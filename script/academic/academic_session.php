...
<head>
<title>MGTechs - Academic Sessions</title>
<meta charset="utf-8">
...
try {
    ...
    if ($active) {
        echo "<div class='alert alert-success'><strong>Active Session:</strong> {$active['session_name']}</div>";
    } else {
        echo "<div class='alert alert-warning'><strong>No active session.</strong> Activate one below.</div>";
    }
} catch (PDOException $e) {
    echo "<div class='alert alert-danger'>".$e->getMessage()."</div>";
}
?>
<!--END  Active Session Display -->
...
<div class="app-title">
  <div><h1>Academic Sessions</h1></div>
  <ul class="app-breadcrumb breadcrumb">
    <li class="breadcrumb-item">
      <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addModal">Add</button>
    </li>
  </ul>
</div>
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title">Add New Session</h5></div>
      <div class="modal-body">
        <form method="POST" action="academic/core/new_academic_session" class="app_frm">
          <div class="mb-3">
            <label class="form-label">Session Name (e.g., 2025/2026)</label>
...
<div class="tile">
  <div class="tile-body">
    <table class="table table-bordered" id="srmsTable">
      <thead>
        <tr>
          <th>Session</th>
          <th>Status</th>
      ...
      <tbody>
        ...
        try {
          $conn = new PDO("mysql:host=".DBHost.";dbname=".DBName.";charset=".DBCharset, DBUser, DBPass);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->query("SELECT * FROM tbl_sessions ORDER BY id DESC");
          while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr>
              <td>{$row['session_name']}</td>
              <td>".($row['is_active'] ? "<span class='badge bg-success'>Active</span>" : "<span class='badge bg-secondary'>Inactive</span>")."</td>
              <td>{$row['created_at']}</td>
              <td>
                <a href='academic/core/activate_session.php?id={$row['id']}' class='btn btn-success btn-sm'>Set Active</a>
                <a href='academic/core/drop_academic_session.php?id={$row['id']}' class='btn btn-danger btn-sm'>Delete</a>
              </td>
            </tr>";
          }
...
<script>$('#srmsTable').DataTable();</script>
</body>
</html>

...
$stmt = $conn->prepare("
    SELECT a.*, c.name AS class_name, t.name AS term_name, s.session_name
    FROM tbl_applications a
    LEFT JOIN tbl_classes c ON a.class_applying_id = c.id
    LEFT JOIN tbl_terms t ON a.term_id = t.id
    LEFT JOIN tbl_sessions s ON a.session_id = s.id
    ORDER BY a.id DESC
");
...
<head>
<meta charset="utf-8">
<title>MGTechs - Manage Student Applications</title>
...
<main class="app-content">
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="tile">
        <div class="tile-body">
          <h3 class="tile-title">Applications List</h3>
            ...
            <table class="table table-bordered" id="applicationsTable">
              ...
              <tbody>
                <?php foreach ($applications as $app): ?>
                  <tr>
                    <td><img src="<?= htmlspecialchars($app['passport']); ?>" class="passport-img"></td>
                    ...
                    <td><?= htmlspecialchars($app['parent_number']); ?></td>
                    <td>
                      <span class="badge bg-<?= $app['status'] == 'Admitted' ? 'success' : 'secondary'; ?>">
                        <?= htmlspecialchars($app['status']); ?>
                      </span>
                    </td>
                    <td>
                      <?php if ($app['status'] != 'Admitted'): ?>
                        <a href="academic/core/approve_application.php?id=<?= $app['id']; ?>" class="btn btn-sm btn-success">
                          <i class="bi bi-check2-circle"></i> Approve
                        </a>
                      <?php else: ?>
                        <button class="btn btn-sm btn-secondary" disabled><i class="bi bi-person-check"></i> Admitted</button>
                      <?php endif; ?>
                    </td>
                  </tr>
                <?php endforeach; ?>

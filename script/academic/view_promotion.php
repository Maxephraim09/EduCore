...
try {
  ...
  $classes = $conn->query("SELECT * FROM tbl_classes ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);
  ...
  if ($selected_session > 0) {
    $stmt = $conn->prepare("
  SELECT p.*, s.fname, s.lname, sess.session_name
  FROM tbl_promotions p
  JOIN tbl_students s ON p.student_id = s.id
  JOIN tbl_sessions sess ON p.session_id = sess.id
  WHERE p.session_id = ?
  ORDER BY p.to_class ASC, s.lname ASC
");
...
<head>
<title>MGTechs - View Promotions</title>
<meta charset="utf-8">
...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1>View Student Promotions</h1>
      <p>Check promoted students by academic session and class.</p>
    </div>
  </div>
  ...
  <div class="tile">
    <form method="GET" class="row g-3">
      ...
      <div class="col-md-3 align-self-end">
        <button type="submit" class="btn btn-primary">View Promotions</button>
      </div>
    </form>
  </div>

  <?php if ($selected_session && count($promotions) > 0): ?>
    <div class="tile">
      <h3>Promotions for Session: 
        <span class="text-success"><?= htmlspecialchars($promotions[0]['session_name']) ?></span>
      </h3>
          ...
          <tbody>
            <?php foreach ($promotions as $i => $p): ?>
              <tr>
                ...
                <td><?= htmlspecialchars($p['from_class']) ?></td>
                <td><?= htmlspecialchars($p['to_class']) ?></td>
                <td><?= htmlspecialchars($p['promoted_on']) ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
  ...
  <?php elseif ($selected_session): ?>
    <div class="alert alert-warning">No promotions recorded for this session yet.</div>
  <?php endif; ?>
</main>

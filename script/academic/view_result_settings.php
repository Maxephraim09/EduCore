...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['session_id']) && isset($_POST['term_id']) && isset($_POST['viewing_status'])) {
    $session_id = $_POST['session_id'];
    $term_id = $_POST['term_id'];
    $status = $_POST['viewing_status'] == "1" ? 1 : 0;

    // Check if record exists
    $stmt = $conn->prepare("SELECT * FROM tbl_view_result WHERE session_id = ? AND term_id = ?");
    $stmt->execute([$session_id, $term_id]);
    $existing = $stmt->fetch();

    if ($existing) {
        // Update existing
        $stmt = $conn->prepare("UPDATE tbl_view_result SET status = ? WHERE session_id = ? AND term_id = ?");
        $stmt->execute([$status, $session_id, $term_id]);
    } else {
        // Insert new
        $stmt = $conn->prepare("INSERT INTO tbl_view_result (session_id, term_id, status, created_at) VALUES (?, ?, ?, NOW())");
        $stmt->execute([$session_id, $term_id, $status]);
    }

    $message = "Result viewing for the selected session and term has been ".($status ? "Enabled" : "Disabled").".";
}

// Get all sessions and terms
$sessions = $conn->query("SELECT * FROM tbl_sessions ORDER BY id DESC")->fetchAll();
$terms = $conn->query("SELECT * FROM tbl_terms WHERE status = 1 ORDER BY id ASC")->fetchAll();

// Get current viewing status for selected session/term (default to first session/term)
...
$current_status = $current_status !== false ? $current_status : 0;
?>
...
<head>
<title>MGTechs - Result Settings</title>
<meta charset="utf-8">
...
<div class="app-title">
    <h1>Manage Result Viewing</h1>
</div>

<div class="row">
<div class="col-md-6">
<div class="tile">
<h4 class="tile-title">Result Viewing Settings</h4>
...
<form method="POST">
    ...
    <div class="form-group mt-2">
        <label>Enable Result Viewing:</label>
        <select name="viewing_status" class="form-control">
            <option value="1" <?php if($current_status==1) echo "selected"; ?>>Enabled</option>
            <option value="0" <?php if($current_status==0) echo "selected"; ?>>Disabled</option>
        </select>
    </div>

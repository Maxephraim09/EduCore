...
try {
    ...
    if ($school) {
        ...
        $school_logo = "images/logo/" . $school['logo'];
    }
    ...
    // Fetch counts
    $academic_terms = 1; // You may replace this with real term count if stored
    $stmt = $conn->query("SELECT COUNT(*) FROM tbl_staff WHERE level = 2 AND status = 1");
    $teachers = $stmt->fetchColumn();

    $stmt = $conn->query("SELECT COUNT(*) FROM tbl_students WHERE status = 1");
    $my_students = $stmt->fetchColumn();

    $stmt = $conn->query("SELECT COUNT(*) FROM tbl_subject_combinations");
    $subjects = $stmt->fetchColumn();

} catch(PDOException $e) {
    $academic_terms = $teachers = $my_students = $subjects = 0;

...
<main class="app-content">
  <div class="app-title">
    <div>
     ...
     <div>
    <a href="teacher/upload_exams_questions"><button class="btn btn-primary app_btn">Submit</button></a>
</div>
  </div>
            ...
            <table class="table table-hover table-bordered" id="srmsTable">
              ...
              <tbody>
                ...
                try {
                    ...
                    // Group by subject so a teacher sees one subject with multiple classes
                    $stmt = $conn->prepare("
                        SELECT 
                            tbl_subjects.name AS subject_name,
                            GROUP_CONCAT(tbl_classes.name SEPARATOR ', ') AS class_list,
                            MAX(tbl_subject_combinations.reg_date) AS added_on
                        FROM tbl_subject_combinations
                        LEFT JOIN tbl_subjects ON tbl_subject_combinations.subject_id = tbl_subjects.id
                        LEFT JOIN tbl_classes ON tbl_subject_combinations.class_id = tbl_classes.id
                        WHERE tbl_subject_combinations.teacher_id = ?
                        GROUP BY tbl_subjects.id
                    ...
                    if ($rows) {
                        foreach ($rows as $row) {
                            ...
                            echo '<td>' . htmlspecialchars($row['class_list']) . '</td>';
                            echo '<td>' . htmlspecialchars($row['added_on']) . '</td>';
                            echo '</tr>';
                        }
                    } else {
                        echo '<tr><td colspan="3" class="text-center text-muted">No subject combinations found.</td></tr>';
                    }

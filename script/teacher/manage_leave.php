...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM tbl_leave_applications WHERE staff_id = ? ORDER BY created_at DESC");
...
<main class="app-content">
  <div class="app-title">
    <div>
     ...
     <div>
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#applyLeaveModal"><i class="bi bi-plus"></i> Apply Leave</button>
</div>
  </div>
...
<table class="table table-bordered table-striped" id="leaveTable">
    ...
    foreach($leaves as $leave){
        $status = $leave['status'] == 0 ? "Pending" : "Approved";
        $days = $leave['number_of_days'] ?? 0;
        $approved_msg = !empty($leave['approved_message']) ? $leave['approved_message'] : "-";

        echo "<tr>
                <td>{$i}</td>
                <td>".htmlspecialchars($leave['user_name'])."</td>
                <td>".htmlspecialchars($leave['purpose'])."</td>
                <td>".htmlspecialchars($leave['start_date'])."</td>
                <td>".htmlspecialchars($leave['returning_date'])."</td>
                <td>{$days}</td>
                <td>{$status}</td>
                <td>".htmlspecialchars($approved_msg)."</td>
...
<div class="modal fade" id="applyLeaveModal" tabindex="-1" aria-labelledby="applyLeaveModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form action="teacher/core/manage_leave.php" method="post" id="leaveForm">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="mb-2">
                ...
                <input type="text" class="form-control" name="user_name" value="<?php echo htmlspecialchars($fname.' '.$lname); ?>" readonly>
            </div>
            <div class="mb-2">
                <label class="form-label">Purpose of Leave</label>
            ...
            <div class="mb-2">
                ...
                <input type="date" class="form-control" name="returning_date" id="returning_date" required>
            </div>
            <div class="mb-2">
                <label class="form-label">Number of Days</label>
                <input type="text" class="form-control" name="number_of_days" id="number_of_days" readonly>
            </div>
        </div>
...
$('#returning_date, #start_date').on('change', function(){
    const start = new Date($('#start_date').val());
    const end = new Date($('#returning_date').val());

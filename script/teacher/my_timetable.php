...
try {
    ...
    // Fetch timetable for classes assigned to this teacher
    $stmt = $conn->prepare("
        SELECT tt.day, tt.start_time, tt.end_time, tt.subject AS subject_name, c.name AS class_name, sess.session_name
        FROM tbl_timetable tt
        JOIN tbl_classes c ON tt.class_id = c.id
        JOIN tbl_sessions sess ON tt.session_id = sess.id
        WHERE tt.class_id IN (
            SELECT DISTINCT class_id FROM tbl_subject_combinations WHERE teacher_id = ?
        )
        ORDER BY FIELD(tt.day, 'Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'), tt.start_time ASC
    ");
    $stmt->execute([$account_id]);
    $all = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Build timetable array using start_time - end_time as the time slot
    foreach($all as $row){
        $slot = date('H:i', strtotime($row['start_time'])) . ' - ' . date('H:i', strtotime($row['end_time']));
        $timetable[$row['day']][$slot] = $row;
...
<style>
    ...
    .timetable-table th, .timetable-table td { border: 1px solid #ccc; padding: 10px; text-align: center; }
    .timetable-table th { background: #007bff; color: #fff; }
    .subject-box { background: #17a2b8; color: #fff; padding: 5px; border-radius: 5px; }
</style>
</head>
            ...
            <?php foreach($days as $day): ?>
                <td>
                    ...
                    if(isset($timetable[$day][$slot])){
                        ...
                        echo "Class: ".htmlspecialchars($row['class_name'])."<br>";
                        echo "Session: ".htmlspecialchars($row['session_name']);
                        echo '</div>';
                    } else {
                        echo '-';
                    }

...
<div class="tile">
<div class="tile-body">
<h3 class="tile-title">Select Class to List Students</h3>
<form class="app_frm" method="POST" autocomplete="OFF" action="">
<div class="mb-2">
...
<button type="submit" name="submit" class="btn btn-primary app_btn">List Students</button>
</form>

<?php
if(isset($_POST['submit'])){
    $class_id = intval($_POST['class_id']);
    $stmt = $conn->prepare("SELECT * FROM tbl_students WHERE class=? AND status=1");
    $stmt->execute([$class_id]);
    $students = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if(count($students) > 0){
        echo "<hr><h4>Students in Selected Class</h4>";
        echo "<table class='table table-bordered' id='studentsTable'>
                <thead>
                <tr>
                    <th>ID</th>
                    <th>Full Name</th>
        ...
        foreach($students as $stu){
            $fullName = $stu['fname'].' '.($stu['mname'] ? $stu['mname'].' ' : '').$stu['lname'];
            echo "<tr>
                    <td>{$stu['id']}</td>
                    <td>{$fullName}</td>
                    <td>{$stu['gender']}</td>
                    <td>{$stu['email']}</td>
                    <td>
                        <a href='teacher/view_student.php?id={$stu['id']}' class='btn btn-sm btn-info'>View</a>
                    </td>
                  </tr>";
        }
        echo "</tbody></table>";
    } else {
        echo "<div class='alert alert-warning mt-2'>No students found in this class.</div>";
    }
}
?>
</div>
...
<script>
$('.select2').select2();
$('#studentsTable').DataTable({"sort": false});
</script>
</body>
</html>

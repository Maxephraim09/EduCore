...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        $promotionEnabled = SystemSetting::getValue('promotion_enabled', 'true');
        $promotionMinAverageScore = SystemSetting::getValue('promotion_min_average_score', '50');
        $promotionMaxAbsenteeism = SystemSetting::getValue('promotion_max_absenteeism', '10');
        $promotionMinPassingSubjects = SystemSetting::getValue('promotion_min_passing_subjects', '7');
        $promotionCoreSubjects = SystemSetting::getValue('promotion_core_subjects', 'English, Mathematics');
        $promotionAutoPromote = SystemSetting::getValue('promotion_auto_promote', 'true');
        ...
        return view('settings.index', compact(
            ...
            'schoolWebsite', 'schoolLogo', 'schoolIcon', 'promotionEnabled',
            'promotionMinAverageScore', 'promotionMaxAbsenteeism', 'promotionMinPassingSubjects',
            'promotionCoreSubjects', 'promotionAutoPromote', 'academicYear', 'term', 'termStartDate', 'termEndDate', 'nextTermStart', 
    ...
    // ==================== ADMISSIONS SETTINGS ====================
    public function updateAdmissionsSettings(Request $request)
    {
        $request->validate([
            'application_fee' => 'nullable|numeric|min:0',
            'applications_open' => 'nullable|in:true,false',
            'registration_fees' => 'nullable|array',
            'admission_letter_content' => 'nullable|string',
            'admission_letter_signatory_name' => 'nullable|string|max:255',
            'admission_letter_signatory_title' => 'nullable|string|max:255',
            'admission_letter_signature' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'remove_admission_letter_signature' => 'nullable|in:true'
        ]);

        try {
            SystemSetting::setValue('application_fee', $request->application_fee ?? 0, 'number', 'Application fee', 'admissions');
            SystemSetting::setValue('applications_open', $request->applications_open ?? 'false', 'select', 'Are applications open', 'admissions');

            $regFees = $request->registration_fees ?? [];
            // sanitize numeric values
            $sanitized = [];
            foreach ($regFees as $classId => $amt) {
                if (is_numeric($amt)) {
                    $sanitized[intval($classId)] = floatval($amt);
                }
            }

            SystemSetting::setValue('registration_fees', json_encode($sanitized), 'json', 'Registration fees by class (class_id => amount)', 'admissions');
            SystemSetting::setValue('admission_letter_content', $request->admission_letter_content ?? '', 'textarea', 'Admission letter content', 'admissions');
            SystemSetting::setValue('admission_letter_signatory_name', $request->admission_letter_signatory_name ?? 'MR. MAXWELL EPHRAIM HALILU', 'text', 'Admission letter signatory name', 'admissions');
            SystemSetting::setValue('admission_letter_signatory_title', $request->admission_letter_signatory_title ?? 'Principal', 'text', 'Admission letter signatory title', 'admissions');

            if ($request->hasFile('admission_letter_signature')) {
                $signature = $request->file('admission_letter_signature');
                $signatureName = 'admission_signature_' . time() . '.' . $signature->getClientOriginalExtension();
                $signature->move(public_path('uploads'), $signatureName);
                SystemSetting::setValue('admission_letter_signatory_signature', 'uploads/' . $signatureName, 'text', 'Admission letter signature image', 'admissions');
            }

            if ($request->input('remove_admission_letter_signature') === 'true') {
                SystemSetting::setValue('admission_letter_signatory_signature', '', 'text', 'Admission letter signature image', 'admissions');
            }

            return redirect()->route('settings.index', ['tab' => 'admissions'])->with('success', 'Admissions settings updated successfully!');
        } catch (\Exception $e) {
            return back()->with('error', 'Error updating admissions settings: ' . $e->getMessage());
        }
    }
    ...
    public function updateAcademicSettings(Request $request)
    {
        $request->validate([
            ...
            'promotion_enabled' => 'nullable|in:true,false',
            'promotion_min_average_score' => 'nullable|numeric|min:0|max:100',
            'promotion_max_absenteeism' => 'nullable|integer|min:0',
            'promotion_min_passing_subjects' => 'nullable|integer|min:0',
            'promotion_core_subjects' => 'nullable|string|max:255',
            'promotion_auto_promote' => 'nullable|in:true,false',
        ]);
        ...
        try {
            ...
            SystemSetting::setValue('promotion_enabled', $request->promotion_enabled ?? 'false', 'select', 'Enable promotions', 'academic');
            SystemSetting::setValue('promotion_min_average_score', $request->promotion_min_average_score ?? '50', 'number', 'Promotion minimum average score', 'academic');
            SystemSetting::setValue('promotion_max_absenteeism', $request->promotion_max_absenteeism ?? '10', 'number', 'Promotion maximum absenteeism', 'academic');
            SystemSetting::setValue('promotion_min_passing_subjects', $request->promotion_min_passing_subjects ?? '7', 'number', 'Promotion minimum passing subjects', 'academic');
            SystemSetting::setValue('promotion_core_subjects', $request->promotion_core_subjects ?? 'English, Mathematics', 'text', 'Promotion core subjects', 'academic');
            SystemSetting::setValue('promotion_auto_promote', $request->promotion_auto_promote ?? 'false', 'select', 'Promotion auto promote', 'academic');
        ...
        try {
            ...
            if (config('database.default') === 'sqlite') {
                ...
                $command = sprintf(
                    ...
                    escapeshellarg((string) $backupFile)
                );
